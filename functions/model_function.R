...
fmm <- function(waves, covariates=NA, data, id=NA, n_chains = 5, iterations = 2500, 
  ...
  if (qtype=="5") {
    m1 <- fmmFive(waves=waves, covariates=covariates, id=id, data=data, n_chains=n_chains, 
             iterations=iterations, burn=burn, cov_estimator=cov_estimator, 
             var_name=var_name)
  } else if (qtype=="3") {
    m1 <- fmmNoStrong(waves=waves, covariates=covariates, id=id, data=data, n_chains=n_chains, 
                iterations=iterations, burn=burn, cov_estimator=cov_estimator, 
                var_name=var_name)
  } else if (qtype=="7") {
    m1 <- fmmSeven(waves=waves, covariates=covariates, id=id, data=data, n_chains=n_chains, 
             iterations=iterations, burn=burn, cov_estimator=cov_estimator, 
             var_name=var_name)
  }
  return(m1)
...
plot.fmm <- function(fmm_object, type = "pattern") {
  if (type == "pattern") {
    pattern_plot(fmm_object)
  } else if (type == "coefficients") {
    coef_plot(fmm_object)
  } else if (type == "pattern_convergence") {
    converge_plot(fmm_object, type = "pattern")
  } else if (type == "coefficient_convergence") {
    converge_plot(fmm_object, type = "coefficient")
  } else if (type == "errors") {
    error_plot(fmm_object)
  }
}

pattern_plot <- function(fmm_object) {
  info=fmm_object[[1]] 
  d <- fmm_object[[3]] 

  title = paste(info$var, ": Pattern parameters", sep = "")
  subtitle = paste("Multinomial regression.", info$iterations, "draws,", info$burn, "burn-in.", "95% CI.")
  ...
  d %>%
    ...
    labs(title = title,
         subtitle = subtitle,
         x="", y = "Coefficient estimates (95%CI)")
}

error_plot <- function(fmm_object) {
  info=fmm_object[[1]] 
  d <- fmm_object[[3]] 
  
  title = paste(info$var, ": Group Standard Errors", sep = "")
  subtitle = paste("Multinomial regression.", info$iterations, "draws,", info$burn, "burn-in.", "95% CI.")
  
  d %>%
    filter(param %in% c("sd_g1", "sd_g2", "sd_g3")) %>%
    mutate(param = recode(param, "sd_g1"="Opinion Holders", "sd_g2"="Vascillating Changers",
                          "sd_g3"="Durable Changers")) %>%
    ggplot(aes(x = param, y = mean)) + 
    geom_linerange(aes(ymin = q25, ymax = q975)) + 
    geom_point(shape = 21, fill = "firebrick") + 
    coord_flip() + 
...
coef_plot <- function(fmm_object) {
  info=fmm_object[[1]] 
  d <- fmm_object[[2]] 
  
  title = paste(info$var, ": Estimated log-odds ratios of group prediction", sep = "")
  subtitle = paste("Multinomial regression.", info$iterations, "draws,", info$burn, "burn-in.", "95% CI.")
  
  d %>%
    filter(param %!in% c("chain", "Intercept_21", "Intercept_23")) %>%
    mutate(param = ifelse(param=="panel_08_23", "panel08_23", param),
           param = ifelse(param=="panel_08_21", "panel08_21", param),
           param = ifelse(param=="panel_10_23", "panel10_23", param),
           param = ifelse(param=="panel_10_21", "panel10_21", param)) %>%
    separate(param, into=c("param", "comp"), sep = "_(?=\\d)") %>%
    mutate(comp = recode(comp, "21"="Stable (g1) over Vascillating (g2)", "23"="Durable Change (g3) over Vascillating (g2)")) %>%
    ggplot(aes(x = param, y = mean)) + 
    ...
    coord_flip() + 
    theme_bw() +
    labs(x = "", y = "Logodds ratio coefficient estimate",
         title = title,
         subtitle = subtitle) + 
    facet_wrap(~comp, scales = "free") 
}

converge_plot <- function(fmm_object, type = "pattern") {
  if (type == "pattern") {
    df <- fmm_object[[5]]
  ...
  info=fmm_object[[1]] 
  df <- df %>% 
    gather(key = "param", value = "value", -c(it, chain)) %>%
    filter(param != "llike") 

  df %>%
    ggplot(aes(x = it, y = value, color = as.factor(chain))) + 
    geom_line(alpha = .5) + 
    facet_wrap(~param, scales = "free") + 
    theme_bw() +
    labs(x = "Iterations", y = "Estimate", color = "Chain") + 
