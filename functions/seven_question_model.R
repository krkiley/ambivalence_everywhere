...
fmmSeven <- function(waves, covariates=NA, id=NA, data, n_chains = 5, iterations = 2500, 
                burn = 500, cov_estimator="multinom", var_name = NA) {
  ...
  for (chain in 1:n_chains) {
    ...
    alpha1=rbeta(1,1,1)
    strength1=rdirichlet(1,c(1,1,1))
    delta1=strength1[1]
    gamma1=strength1[2]
    strength2=rdirichlet(1,c(1,1,1))
    delta2=strength2[1]
    gamma2=strength2[2]
    tau3=rbeta(1,1,1)
    alpha3post=rbeta(1,1,1)
    strength3=rdirichlet(1,c(1,1,1))
    delta3=strength3[1]
    gamma3=strength3[2]
    ...
    Y_star <- as.matrix(Y)
    Y_star[is.na(Y_star[,1]),1] <- sample(1:7, sum(is.na(Y_star[,1])), replace = TRUE)
    Y_star[is.na(Y_star[,2]),2] <- sample(1:7, sum(is.na(Y_star[,2])), replace = TRUE)
    Y_star[is.na(Y_star[,3]),3] <- sample(1:7, sum(is.na(Y_star[,3])), replace = TRUE)
    ...
    for (j in 1:iterations) {
      ...
      Ai = ifelse(Y_star[,1] %in% c(5,6,7), 1, 0)
      Bi = ifelse(Y_star[,1] %in% c(1,7), 1, 0) + ifelse(Y_star[,2] %in% c(1,7), 1, 0) +
        ifelse(Y_star[,3] %in% c(1,7), 1, 0)
      Si = ifelse(Y_star[,1] %in% c(3,5), 1, 0) + ifelse(Y_star[,2] %in% c(3,5), 1, 0) +
        ifelse(Y_star[,3] %in% c(3,5), 1, 0)
      Ci = ifelse(Y_star[,1]==4,1,0) + ifelse(Y_star[,2]==4,1,0) +
        ifelse(Y_star[,3]==4,1,0)
      Di = ifelse(Y_star[,1] < 4 & Y_star[,2] >= 4, 1, 0) +
        ifelse(Y_star[,1] > 4 & Y_star[,2] <= 4, 1, 0) +
        ifelse(Y_star[,2] < 4 & Y_star[,3] >= 4, 1, 0) +
        ifelse(Y_star[,2] > 4 & Y_star[,3] <= 4, 1, 0) +
        ifelse(Y_star[,1] == 4 & Y_star[,2] != 4, 1, 0) +
        ifelse(Y_star[,2] == 4 & Y_star[,3] != 4, 1, 0)
      Ei = ifelse(Di != 1, 0, 
                  ifelse((Y_star[,1] < 4 & Y_star[,2] >= 4) | 
                           (Y_star[,1] == 4 & Y_star[,2] != 4) |
                           (Y_star[,1] > 4 & Y_star[,2] <= 4), 1, 2))
      Fi = ifelse(Di != 1 | (Ei == 1 & Y_star[,1] %in% c(1,2,3,4)) |
                    (Ei == 2 & Y_star[,2] %in% c(1,2,3,4)), 0, 1)
      Hi = ifelse(Di == 1 & Y_star[,1] == 4, 1, 0)
      Mi = ifelse(Di == 1 & Y_star[,3] %in% c(5,6,7), 1, 0)
      Qi = ifelse(Di == 1 & Y_star[,3] == 4, 1, 0)
      Ri = ifelse(Y_star[,1] %in% c(1,2,3), 1, 0) + ifelse(Y_star[,2] %in% c(1,2,3), 1, 0) +
        ifelse(Y_star[,3] %in% c(1,2,3), 1, 0) 
      ...
      p1=(alpha1^(1-Ai)*(1-alpha1)^Ai*delta1^Bi*gamma1^Si*(1-delta1-gamma1)^(3-Bi-Si))*
        ifelse(Ci==0,1,0)*ifelse(Di==0,1,0)
      p2=phi2^Ci*alpha2^Ri*(1-phi2-alpha2)^(3-Ci-Ri)*delta2^Bi*gamma2^Si*
        (1-delta2-gamma2)^(3-Ci-Bi-Si)
      ...
      strength1=rdirichlet(1,c( (sum(Bi[g1==1])/3)+(2/3), (sum(Si[g1==1])/3)+(2/3), 
                                sum(g1)-(sum(Bi[g1==1])/3)-(sum(Si[g1==1])/3)+(2/3)))
      delta1=strength1[1]
      gamma1=strength1[2]
      ...
      phi2=two_params[1]
      alpha2=two_params[2]
      strength2=rdirichlet(1,c( (sum(Bi[g2==1])/3)+(2/3),
                                (sum(Si[g2==1])/3)+(2/3),
                                sum(g2)-(sum(Ci[g2==1])/3)-(sum(Bi[g2==1])/3)-
                                  (sum(Si[g2==1])/3)+(2/3) ))
      delta2=strength2[1]
      gamma2=strength2[2]
      ...
      strength3=rdirichlet(1,c((sum(Bi[g3==1])/3) + (2/3),
                               (sum(Si[g3==1])/3) + (2/3),
                               sum(g3) - (sum(Ci[g3==1])/3) - (sum(Bi[g3==1])/3) -
                                 (sum(Si[g3==1])/3) + (2/3)))
      delta3=strength3[1]
      gamma3=strength3[2]
      ...
      GY <- cbind(G, Y)
      Y_star[Ymiss,] <- t(apply(GY[Ymiss,], 1, logicSeven, alpha1, 
                                delta1, gamma1, phi2, alpha2, delta2, 
                                gamma2,
                                phi3pre1, phi3pre2, alpha3pre1, alpha3pre2, 
                                alpha3post, delta3, gamma3, tau3))
      ...
      pattern_params[[count]]=data.frame(it=j, chain=chain, pi1=pi1, pi2=pi2, pi3=pi3, 
                                         alpha1=alpha1, delta1=delta1, gamma1=gamma1,
                                         ...
                                         delta3=delta3, gamma3=gamma3, tau3=tau3, 
  ...
  model_info = list(var = var_name, qtype="7", n_chains = n_chains, iterations = iterations, 
...
logicSeven <- function(vec, alpha1, delta1, gamma1, phi2, alpha2, delta2, 
                           gamma2,
                           phi3pre1, phi3pre2, alpha3pre1, alpha3pre2, 
                           alpha3post, delta3, gamma3, tau3) {
  g <- vec[1]
  ...
  if (g==1) {
    if (y[!is.na(y)][1] < 4) {
      y[is.na(y)] <- sample(c(1,2,3), sum(is.na(y)), replace=T,
                            prob=c(delta1, 1-delta1-gamma1, gamma1))
    } else {
      y[is.na(y)] <- sample(c(7,6,5), sum(is.na(y)), replace=T,
                            prob=c(delta1, 1-delta1-gamma1, gamma1))
    }
  } else if (g==2) {
    y[is.na(y)] <- 
      sample(1:7, sum(is.na(y)), replace=T, 
             prob=c(alpha2*delta2,alpha2*(1-delta2-gamma2),alpha2*gamma2,
                    phi2,
                    (1-alpha2)*gamma2,
                    (1-alpha2)*(1-delta2-gamma2),
                    (1-alpha2)*delta2))
  } else if (g==3) {
    if (sum(is.na(y)==c(0,1,1))==3) {
      if (rbinom(1,1,tau3)) {
        if (y[1]==4) {
          if (rbinom(1,1,alpha3post)) {
            y[c(2,3)] <- sample(c(1,2,3),2,replace=T,prob=c(delta3,1-delta3-gamma3,gamma3))
          } else {
            y[c(2,3)] <- sample(c(7,6,5),2,replace=T,prob=c(delta3,1-delta3-gamma3,gamma3))
          }
        } else if (y[1] %in% c(1,2, 3)) {
          y[c(2,3)] <- sample(c(7,6,5),2,replace=T,prob=c(delta3,1-delta3-gamma3,gamma3))
        } else {
          y[c(2,3)] <- sample(c(1,2,3),2,replace=T,prob=c(delta3,1-delta3-gamma3,gamma3))
        }
        ...
        if (y[1]==4) {
          y[c(2,3)] <- c(3,sample(c(1,2,3,5,6,7),1,
                                  prob=c(alpha3post*delta3,alpha3post*(1-delta3-gamma3),
                                         alpha3post*gamma3,
                                         (1-alpha3post)*gamma3,
                                         (1-alpha3post)*(1-delta3-gamma3),
                                         (1-alpha3post)*delta3)))
        } else if (y[1] %in% c(1,2,3)) {
          y[c(2,3)] <- c(sample(c(1,2,3),1,prob=c(delta3,1-delta3-gamma3,gamma3)),
                         sample(c(7,6,5),1,prob=c(delta3,1-delta3-gamma3,gamma3)))
        } else {
          y[c(2,3)] <- c(sample(c(7,6,5),1,prob=c(delta3,1-delta3-gamma3,gamma3)),
                         sample(c(1,2,3),1,prob=c(delta3,1-delta3-gamma3,gamma3)))
        }
      }
    } else if (sum(is.na(y)==c(0,0,1))==3) {
      if (y[1]==4&y[2]==4) {
        y[3] <- sample(c(1,2,3,5,6,7),1,
                       prob=c(alpha3post*delta3,alpha3post*(1-delta3-gamma3),
                              alpha3post*gamma3,
                              (1-alpha3post)*gamma3,
                              (1-alpha3post)*(1-delta3-gamma3),(1-alpha3post)*delta3))
      } else if ((y[1]%in%c(1,2,3,4)&y[2]%in%c(5,6,7)) | 
                 (y[1]%in%c(1,2,3)&y[2]%in%c(1,2,3))) {
        y[3] <- sample(c(7,6,5),1,prob=c(delta3,1-delta3-gamma3,gamma3))
      } else {
        y[3] <- sample(c(1,2,3),1,prob=c(delta3,1-delta3-gamma3,gamma3))
      } 
    } else if (sum(is.na(y)==c(0,1,0))==3) {
      if (rbinom(1,1,tau3)) {
        if (y[3] %in% c(5,6,7)) {
          y[2] <- sample(c(7,6,5),1,prob=c(delta3,1-delta3-gamma3,gamma3))
        } else {
          y[2] <- sample(c(1,2,3),1,prob=c(delta3,1-delta3-gamma3,gamma3))
        }
      } else {
        if (y[1]==4) {
          y[2] <- 4
        } else if (y[3] %in% c(5,6,7)) {
          y[2] <- sample(c(1,2,3),1,prob=c(delta3,1-delta3-gamma3,gamma3))
        } else {
          y[2] <- sample(c(7,6,5),1,prob=c(delta3,1-delta3-gamma3,gamma3))
        }
      ...
      if (y[2]==4) {
        ...
        if (rbinom(1,1,tau3)) {
          if (y[2]%in%c(1,2,3)) {
            y[c(1,3)] <- c(sample(c(4,5,6,7),1,prob=c(phi3pre1,(1-phi3pre1)*gamma3,
                                                      (1-phi3pre1)*(1-delta3-gamma3),
                                                    (1-phi3pre1)*delta3)),
                           sample(c(1,2,3),1,prob=c(delta3,1-delta3-gamma3,gamma3)))
          } else {
            y[c(1,3)] <- c(sample(c(4,3,2,1),1,prob=c(phi3pre1,(1-phi3pre1)*gamma3,
                                                    (1-phi3pre1)*(1-delta3-gamma3),
                                                    (1-phi3pre1)*delta3)),
                           sample(c(7,6,5),1,prob=c(delta3,1-delta3-gamma3,gamma3)))
          }
          ...
          if (y[2]%in%c(1,2)) {
            y[c(1,3)] <- c(sample(c(1,2,3),1,prob=c(delta3,1-delta3-gamma3,gamma3)),
                           sample(c(7,6,5),1,prob=c(delta3,1-delta3-gamma3,gamma3)))
          } else {
            y[c(1,3)] <- c(sample(c(7,6,5),1,prob=c(delta3,1-delta3-gamma3,gamma3)),
                           sample(c(1,2,3),1,prob=c(delta3,1-delta3-gamma3,gamma3)))
          }
        }
      }
      ...
      if (y[2]==4) {
        y[1] <- 4
      } else if (y[2] %in% c(1,2,3) & y[3] %in% c(1,2,3)) {
        y[1] <- sample(c(4,5,6,7),1,
                       prob=c(phi3pre1,(1-phi3pre1)*gamma3,
                              (1-phi3pre1)*(1-delta3-gamma3),
                              (1-phi3pre1)*delta3))
      } else if (y[2] %in% c(1,2,3) & y[3] %in% c(5,6,7)) {
        y[1] <- sample(c(1,2,3),1,prob=c(delta3,1-delta3-gamma3,gamma3))
      } else if (y[2] %in% c(5,6,7) & y[3] %in% c(1,2,3)) {
        y[1] <- sample(c(7,6,5),1,prob=c(delta3,1-delta3-gamma3,gamma3))
      } else if (y[2] %in% c(7,6,5) & y[3] %in% c(7,6,5)) {
        y[1] <- 
          sample(c(4,3,2,1),1,prob=c(phi3pre1,(1-phi3pre1)*gamma3,
                                   (1-phi3pre1)*(1-delta3-gamma3),
                                   (1-phi3pre1)*delta3))
      }
      ...
      if (rbinom(1,1,tau3)) {
        if (y[3]%in%c(1,2,3)) {
          y[1] <- sample(c(4,5,6,7),1,prob=c(phi3pre1,(1-phi3pre1)*gamma3,
                                             (1-phi3pre1)*(1-delta3-gamma3),
                                             (1-phi3pre1)*delta3))
          y[2] <- sample(c(1,2,3),1,prob=c(delta3,1-delta3-gamma3,gamma3))
        } else {
          y[1] <- sample(c(4,3,2,1),1,prob=c(phi3pre1,(1-phi3pre1)*gamma3,
                                             (1-phi3pre1)*(1-delta3-gamma3),
                                             (1-phi3pre1)*delta3))
          y[2] <- sample(c(7,6,5),1,prob=c(delta3,1-delta3-gamma3,gamma3))
        }
